...
//							//JOptionPane.showMessageDialog(null, "No Name of the online users matches this name : "+ data[3] +"\n Check the online users correctly ", "This user is not found or not online", JOptionPane.ERROR_MESSAGE);
                             ...
                             for (String current_user : users)
                     {
                             ...
                             //JOptionPane.showMessageDialog(null, " am here ,,,,, " + finalString);
                    }
                    else 
                    {
    ...
    private void initComponents() {
        ...
        b_start.setBackground(Color.ORANGE);
        ...
        b_users.setBackground(Color.ORANGE);
        b_clear = new javax.swing.JButton();
        b_clear.setFont(new Font("Tahoma", Font.BOLD, 11));
        b_clear.setBackground(Color.ORANGE);

        setDefaultCloseOperation(javax.swing.WindowConstants.EXIT_ON_CLOSE);
        setTitle("Chat ---- Server Frame");
        setName("server"); 
        setResizable(false);
        ...
        pack();
    }// init components ends

    private void b_endActionPerformed(java.awt.event.ActionEvent evt) {//Close Button method
        try 
        {
           // Thread.sleep(500);                 
            ta_chat.append("\n");
    ...
    private void b_startActionPerformed(java.awt.event.ActionEvent evt) {//Start button method
        Thread starter = new Thread(new ServerStart());
        starter.start();
        ta_chat.setEditable(false); //when server start text cannot be written
        ta_chat.append(" Server has been started \n Waiting for connection ...");
        changetxtareafontsize(ta_chat) ;
    }

    private void b_usersActionPerformed(java.awt.event.ActionEvent evt) {//online users method
        ta_chat.append("\n Online users : \n");
        if(!users.isEmpty())
        {
    ...
    private void b_clearActionPerformed(java.awt.event.ActionEvent evt) {//Clear button method
        ta_chat.setText("");
    }
...
//				clientOutputStreams[id]= writer;
    ...
    public void userAdd (String data) 
    {
        ...
        {
            message = (token + s); // name + String connect
            tellEveryone(message);
        }
        tellEveryone(done);
    }
